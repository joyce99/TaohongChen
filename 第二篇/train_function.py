...
def train(args,model, data_loader, train_attrbs, optimizer, use_cuda,epoch,lamb_1=1.0):
    ...
    for batch_idx, (data, label) in enumerate(tk):
        ...
        x_g,x_aux,aux,token_cls_reg,*rest = model.vit(data,keep_rate,label=label,att=train_attrbs)  #
        print("x_g的维度",x_g.shape)

        feat_g = model.mlp_g(x_g)
        print("mlp_g输出的维度为",feat_g.shape)
        print("train_attrbs的维度为",train_attrbs.shape)
