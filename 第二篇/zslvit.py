...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., keep_rate=1.):
        super().__init__()
        self.num_heads = num_heads #多头数为8
        head_dim = dim // num_heads #每个头的维度
        self.scale = head_dim ** -0.5

        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias) #一个线性层，将输入特征映射到Q,K,V，输出维度为dim*3
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)
        self.keep_rate = keep_rate #设置保留率
        assert 0 < keep_rate <= 1, "keep_rate must > 0 and <= 1, got {0}".format(keep_rate) #语句检查keep_rate是否在（0，1]范围内，不在则抛出异常

    # 神经网络的前馈传播，根据保留率或指定的token数量保留
    def forward(self, x, keep_rate=None, tokens=None,aux_sem = None):
        ...
        if aux_sem is not None:
            q[:,:,0,:] = 0.9*q[:,:,0,:]+ 0.1*aux_sem # 将其与Q矩阵的第一个token进行加权平均
...
class ZSLViT(nn.Module):
    ...
    def __init__(self,img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if self.dataset == 'CUB':
            atttovis_list = [SemanticTrans(312,embed_dim) for _ in range(3)] 
            self.atttovis = nn.ModuleList(atttovis_list)
            token_reg_list = [Token_cls_reg(embed_dim,312) for _ in range(3)]
            self.token_cls_reg = nn.ModuleList(token_reg_list)
        elif self.dataset == 'SUN':
            atttovis_list = [SemanticTrans(102,embed_dim) for _ in range(3)] 
            self.atttovis = nn.ModuleList(atttovis_list)
            token_reg_list = [Token_cls_reg(embed_dim,102) for _ in range(3)]
            self.token_cls_reg = nn.ModuleList(token_reg_list)
        else:
            atttovis_list = [SemanticTrans(85,embed_dim) for _ in range(3)] 
            self.atttovis = nn.ModuleList(atttovis_list)
            token_reg_list = [Token_cls_reg(embed_dim,85) for _ in range(3)]
            self.token_cls_reg = nn.ModuleList(token_reg_list)
        # atttovis_list = [SemanticTrans(102,embed_dim) for _ in range(3)] 
        # self.atttovis = nn.ModuleList(atttovis_list)
        # token_reg_list = [Token_cls_reg(embed_dim,102) for _ in range(3)]
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False,label = None,att=None):
        ...
        for i, blk in enumerate(self.blocks):
            if self.keep_rate[i] < 1 and self.training and att is not None: 
               aux_sem = self.atttovis[p_count](att[label]).reshape(-1,self.num_heads, self.embed_dim // self.num_heads)
               aux.append(aux_sem.reshape(-1,self.embed_dim))
               x, left_token, idx,token_cls = blk(x, keep_rate[i], tokens[i], get_idx,aux_sem) 
...
@torch.no_grad()
def _load_weights(model: ZSLViT, checkpoint_path: str, prefix: str = ''):
...
@register_model
def zslvit_base(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    keep_rate = [1] * 12
    for loc in drop_loc:
